...
from src.courier.mysql_courier import MysqlCourier
from src.job_attributes import JobStatus, JobOperate
    ...
    def __init__(self, json_params):
        try:
            ...
            if type(self.src_tables) == list:
                    loger.print_info("tablename type is list")
            else:
                loger.print_waring("convert tablename type to list")
            ...
            loger.print_info("Finish parser")
        except Exception as error_message:
            loger.print_error("Parser The Paramas Error, " + str(error_message))
            importjob = db_conns.importjob_conn.update_status(self.jobid, JobStatus.EXCEPTION)
            raise error_message


class MysqlTask(TaskBasc):
    def __init__(self, jobid, jparams):
        loger = logger.Logger("Method:init")
        self.params = mysql_jparser(jparams)
        self.jobid = jobid
    ...
    def run(self):
        ...
        mysql_obj = MysqlClient(self.params.json_params)
        loger.print_info("Mysql DB Connection Successfully")
        for table in self.params.src_tables:
            ...
            if self.params.operation_flag == 1 or self.params.operation_flag == '1':
                ...
                for i in range(len(pretreate_sqls)):
                    loger.print_info("Pre_SQL is :%s"%(pretreate_sqls[i]))
                ...
                sqls = hive_obj.mysql_create_hiveTable(self.params.dst_database, self.params.prefix + table, table, mysql_obj, self.params.jobid)
                ...
                MysqlCourier.sqoop_data(self.params.src_hostname, self.params.src_port, self.params.src_databases, self.params.src_username, self.params.src_password, self.params.jobid, self.params.dst_database, self.params.prefix + table,  table, mysql_obj)
                loger.print_info("Finish sqoop import data into hive db")
                loger.print_info("Start PXF-Service import into HDB")
                table_sql = MysqlCourier.mysql_pxf_write_table(mysql_obj, self.params.prefix + table, table, self.params.dst_hostname, self.params.dst_database, self.params.operation_flag, self.params.jobid)
                ...
                if self.params.increment_flag == 1 or self.params.increment_flag == '1':
                    ...
                    MysqlCourier.sqoop_data_hbase(self.params.src_hostname, self.params.src_port, self.params.src_databases, self.params.src_username, self.params.src_password, self.params.jobid, self.params.dst_database, self.params.prefix + table, table, self.params.determine_field, self.params.determine_value)
                    loger.print_info("Finish sqoop import data into hbase db")
                    loger.print_info("Start PXF-Service import into HDB")
                    table_sql = MysqlCourier.mysql_hbase_pxf_write_table(mysql_obj, self.params.prefix + table, table, self.params.dst_hostname, self.params.dst_database, self.params.jobid, self.params.determine_type, self.params.determine_field)
                    empty_external_sql = 'drop external table "%s_ext"'%(self.params.prefix + table)
                    loger.print_info("%s:"%(empty_external_sql))
                    hawq_obj.execute_sql(empty_external_sql)
                    empty_sql = 'drop table "%s"'%(self.params.prefix + table)
